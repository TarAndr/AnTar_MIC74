...
void setup() {
  ...
  int portModeValue = mic.readPortOutMode();   // Reading the current value of port out mode register

  // Preparing a Port Value to Set a Specific Pin Mode
  portModeValue = mic.bitToSet(portModeValue, ledPin1);    // Setting a port out bit to set it to pushpull
  portModeValue = mic.bitToSet(portModeValue, ledPin2);    // Setting a port out bit to set it to pushpull
  portModeValue = mic.bitToSet(portModeValue, ledPin3);    // Setting a port out bit to set it to pushpull
  portModeValue = mic.bitToSet(portModeValue, ledPin4);    // Setting a port out bit to set it to pushpull

  // Write a new port value to the mode register
  mic.writePortOutMode(portModeValue);   // led pins mode as an pushpull

  portModeValue = mic.readPortMode();   // Reading the current value of port mode register
